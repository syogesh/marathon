...
trait PersistentStoreTest extends IntegrationFunSuite with Matchers with BeforeAndAfter {
  ...
  test("Multiple updates will update only with correct version") {
    ...
    persistentStore.update(read.get.mutate("Hello again".getBytes)).futureValue.bytes should be("Hello again".getBytes)
    whenReady(persistentStore.update(read2.get.mutate("Will be None".getBytes)).failed) { _ shouldBe a[StoreCommandFailedException] }
    val readAgain = fetch("foo")
    readAgain.get.bytes should be("Hello again".getBytes)
  }
